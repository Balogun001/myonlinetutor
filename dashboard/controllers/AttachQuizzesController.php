...
class AttachQuizzesController extends DashboardController
{
    ...
    public function searchattempedquiz(){
        $frm = QuizSearch::getQuizForm();
        $post = $frm->getFormDataFromArray(FatApp::getPostedData(), ['quilin_quiz_id']);
        $db = FatApp::getDb();
        $query = $db->query("SELECT quilin_id FROM tbl_quiz_linked WHERE quilin_quiz_id = ".$post['quilin_quiz_id'][0]." AND quilin_record_id = ".$post['quilin_record_id']." AND quilin_user_id = ".$post['quilin_user_id']." AND quilin_record_type=".$post['quilin_record_type'] ." AND quilin_deleted IS NULL ORDER BY quilin_id DESC LIMIT 1");
        $data = $db->fetch($query);
        $query2 = $db->query("SELECT quizat_id FROM tbl_quiz_attempts WHERE quizat_quilin_id = ".$data['quilin_id']." ORDER BY quizat_id DESC LIMIT 1");
        $data2 = $db->fetch($query2);
        echo $data2['quizat_id'];exit;
    }

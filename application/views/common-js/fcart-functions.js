...
var cart = {
	...
	gradeSelection_checkbtn: function (teacherId, tlangId, duration,checkbtn) {

		sessionStorage.setItem('btn',checkbtn);
        cart.prop.ordles_teacher_id = teacherId;
        cart.prop.ordles_tlang_id = (tlangId != '') ? tlangId : parseInt($('input[name="ordles_tlang_id"]:checked').val());
        cart.prop.ordles_duration = (duration != '') ? duration : parseInt($('input[name="ordles_duration[' + cart.prop.ordles_tlang_id + ']"]:checked').val());
        fcom.process();
    ...
    confirmOrder: function (form) {
       /* if (!$(form).validate()) {
        ...
        //form.submit.disabled = true;
		 $.ajax({url: fcom.makeUrl("Cart", "confirmOrder"),type: 'POST',data:{'data':fcom.frmData(form)}, success: function(response){
			 setTimeout(function () {
	...
	stripe: function (form) {
		/*if (!$(form).validate()) {
			return;
		}*/
        //fcom.process();
        /*fcom.updateWithAjax(fcom.makeUrl("Cart", "stripe"), fcom.frmData(form), function (response) {
            if (response.redirectUrl) {
                window.location.href = response.redirectUrl;
            }
            
        }, {failed: true});*/
		 $.ajax({url: fcom.makeUrl("Cart", "stripe"),type: 'POST',data:{'data':fcom.frmData(form)}, success: function(response){
			var obj = jQuery.parseJSON(response);
			if (!obj.error) {
                window.location.href = obj.redirectUrl;
            }else{
